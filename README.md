...
- Phase 8 / 9: 画像認識・カロリー推定の納得感改善、キャロリン音声読み上げ

DB保存、ログイン、決済、有料TTS、本格3D/Live2D実装はまだ入れていません。
...
- Phase 8では、複数品目の分解、品目別カロリー範囲、不確実性メモを返す形式にしています。
...
Phase 9では、ブラウザ標準のWeb Speech APIでキャロリンの `voiceLine` または短縮した煽り文を読み上げる仮音声機能を追加しています。

- 外部TTSや有料音声APIはまだ使っていません。
- 日本語音声がある環境では日本語音声を優先します。
- ブラウザやOSによって声質、利用可否、再生挙動が変わります。
- Web Speech APIは仮実装です。需要検証後に商用利用可能なTTSへ移行する前提です。
- 特定の実在声優、配信者、既存キャラクターの声に寄せる実装はしていません。
...
11. キャロリントーク画面の「音声で聞く」で、短いセリフが再生/停止できることを確認する。
...
- ショート動画で需要検証する。
- 次フェーズでスマホ画面録画と少数の実AI解析テストを行い、精度、料金、レスポンス時間を確認する。
